...
namespace AddressInfoApp
{
    public partial class FrmMain : Form
    {
        ...
        private void BtnInsert_Click(object sender, EventArgs e)
        {
            ...
            if (result > 0) // 원래 있던 데이터를 insert하는 버그 방지
            {
                MessageBox.Show("초기화를 하십시오.");
                return;
            }

            if (string.IsNullOrEmpty(TxtFullName.Text) || string.IsNullOrEmpty(TxtMobile.Text)) // null 데이터 insert 방지
                {
        ...
        {
            var selData = DgvAddress.Rows[e.RowIndex].Cells; //Cells 한 행을 다 읽어옴
